...
export default class CoinService{
    ...
    constructor(){
        ...
        this._http = axios.create({ baseURL: urlApi }) 
   ...
   filterCoins(filter: string){
    ...
    return this._coins.filter((c) => {return c.name.toLowerCase().includes(sanitizedFilter) || c.symbol.toLowerCase().includes(sanitizedFilter)})
   }
}